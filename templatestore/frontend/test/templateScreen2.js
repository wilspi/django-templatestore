...
class TemplateScreen extends React.Component {
    ...
    handleOutputChange(e) {
        ...
        this.setState({
            valueTemplate: defaultValue.dhtmldoc,
            type: e.target.value
        });
    }

    loadData(templateName, templateVersion) {
        axios.get(`/templatestore/api/v1/template/${templateName}/${templateVersion}`).then(response => {
        ...
        axios.get(`/templatestore/api/v1/template/${templateName}/versions`).then(response => {
            this.setState({
    ...
    setDefaultVersion(templateName, templateVersion) {
        axios.post(`/templatestore/api/v1/template/${templateName}/${templateVersion}`, {
            default: true
        }).then(response => {
    ...
    saveTemplate() {
        axios.post("/templatestore/api/v1/template", {
    ...
    updateAttributes() {
        axios.put(`/templatestore/api/v1/template/${this.state.templateName}`, {
            attributes: this.state.attr
        }).then(response => {
            this.setState({
                updateOrNot: false
    ...
    render() {
        let attributes = this.state.attributes.map((attribute) => {
            ...
            rows.push(<td><input className={styles.teInputField} type="text" name="attribute" id="attribute" /></td>);
            rows.push(<td><input className={styles.teInputField} type="text" name="value" id="value" /></td>);
            return (
                <tr>
                    {rows}
                </tr>
        ...
        let versions = this.state.versionList.map((versions) => {
            let rows = [];
            let defaultButton = <button className={styles.teButtons} type="button" onClick={this.setDefaultVersion.bind(this, this.state.templateName, versions.version)}> Set As Default </button>;
            ...
            if (this.state.templateVersion === versions.version) {
            ...
            rows.push(
                <td align="center">
                    <Link to={`/templatestore/${this.state.templateName}/${versions.version}`}>
                        {OpenOrNot}
                    </Link>
                </td>);
        ...
        let textOutput = <AceEditor height="0px" width="0px"/>;
        if (this.state.type === "text") {
            textOutput = (<AceEditor
                name="output-editor"
        ...
        let htmlOutput = <iframe height="0px" width="0px"/>;
        if (this.state.type === "html") {
            htmlOutput = <iframe height={this.state.height} width={this.state.width} srcDoc={this.state.valueOutput}/>;
        }
            ...
            <div className={styles.teScreen}>
                ...
                <div className={styles.teScreenTable}>
                    ...
                    <div className={styles.teInputCol}>
                        ...
                        <div>
                            ...
                            <select id="type" className={styles.teButtons} value={this.state.type} onChange={this.handleOutputChange.bind(this)}>
                ...
                <div>
                    {
                        this.state.attributes.map((attribute) => {
                            for (let key in attribute) {
                                <attributesBlock key={key} value={attribute[key]} />;
                            }
                        })
                    }
                </div>
                ...
                <div>
                    ...
                    <div align="center">
                        ...
                        <div align="center">
                            <button className={styles.teButtons} onClick={this.addAttributes.bind(this)}>+</button>
                            {updateButton}
                        </div>
                    </div>
                </div>
